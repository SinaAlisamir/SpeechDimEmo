...
def main(sessionID):
    ...
    VADFolder = os.path.join(saveFolder, "VAD")
    update_sec = 0.01
    PlotVAD(savePathBuffer, update_sec, VADFolder)
...
def loadVAD(VADFolder):
    vadOutsPath = os.path.join(VADFolder, 'VAD_outs.csv')
    time.sleep(0.000001) # animate function did not work without this line!
    vadOuts = np.loadtxt(vadOutsPath, dtype='float', delimiter=',', usecols=(0), unpack=True)
    return vadOuts

def PlotVAD(savePathBuffer, update_sec, VADFolder):
    ...
    plt.legend(["Audio Input", "VAD Output"])
    plt.xlabel('Time')
